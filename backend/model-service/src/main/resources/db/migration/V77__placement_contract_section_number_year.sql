update allu.configuration set type='TEXT', key='PLACEMENT_CONTRACT_SECTION_NUMBER_YEAR' where key='placement_contract_section_number_year';
...
BEGIN
  ...
  if (year is null or year <> cur_year) then
    ...
    if (conf_count < 1) then
      insert into allu.configuration (key, type, value) values ('PLACEMENT_CONTRACT_SECTION_NUMBER_YEAR', 'TEXT', cur_year);
    else
