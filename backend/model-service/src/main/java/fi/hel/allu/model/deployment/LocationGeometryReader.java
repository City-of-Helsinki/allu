...
public class LocationGeometryReader {
  ...
  private static final String SQL_COMMENTS = "-- DO NOT EDIT THIS FILE MANUALLY!\n\n"
  ...
  // Find out the java commandline that was used
  ...
  private void insertIntoDb(Feature feature, OutputStream output) throws IOException {
    ...
    BooleanExpression areaCondition;
    if (exactMatch) {
      areaCondition = fixedLocation.area.eq(area);
    } else {
      areaCondition = fixedLocation.area.like(area);
    }
    String insert = queryFactory.update(fixedLocation).set(fixedLocation.geometry, geometry)
        .where(areaCondition.and(sectionCondition)).getSQL().get(0).getSQL();
    output.write((insert + ";\n").getBytes());
  }
}
