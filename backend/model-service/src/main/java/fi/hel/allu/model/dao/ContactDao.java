...
public class ContactDao {
  ...
  public Page<Contact> findAll(Pageable pageRequest) {
    ...
    List<Expression<?>> mappedExpressions = new ArrayList<>(Arrays.asList(contact.all()));
    mappedExpressions.add(bean(PostalAddress.class, postalAddress.all()).as("postalAddress"));
    List<Contact> results = queryFactory
        .select(Projections.bean(Contact.class, mappedExpressions.toArray(new Expression[0])))
        .from(contact)
        .leftJoin(postalAddress).on(contact.postalAddressId.eq(postalAddress.id))
  ...
  /**
   * Performs a soft delete operation on contacts by setting their "isActive" attribute to false.
   * This method updates the status of the contacts with the specified IDs in the database.
   *
   * @param ids a list of contact IDs to be soft deleted; must not be null or empty
   * @return the number of contacts that were successfully updated as part of the soft delete operation
   */
  public long softDeleteContactsByIds(List<Integer> ids) {
