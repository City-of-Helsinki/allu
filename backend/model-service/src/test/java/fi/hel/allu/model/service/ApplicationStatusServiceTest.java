...

  {
    ...
    describe("Status change operations", () -> {
      describe("Change status", () -> {
        ...
        beforeEach(() -> {
          when(applicationService.changeApplicationStatus(app.getId(), StatusType.HANDLING, app.getHandler()))
        ...
        it("Should change status using application service", () -> {
          applicationStatusService.changeApplicationStatus(app.getId(), StatusType.HANDLING, app.getHandler());
          Mockito.verify(applicationService, Mockito.times(1))
              .changeApplicationStatus(eq(app.getId()), eq(StatusType.HANDLING), eq(app.getHandler()));
        });
      });
      ...
      describe("Change status to DECISION", () -> {
        ...
        beforeEach(() -> {
          when(applicationService.changeApplicationStatus(app.getId(), StatusType.DECISION, app.getHandler()))
              .thenReturn(mockApp);
        ...
        it("should change status but not update location", () -> {
          applicationStatusService.changeApplicationStatus(app.getId(), StatusType.DECISION, app.getHandler());

          Mockito.verify(applicationService, Mockito.times(1))
              .changeApplicationStatus(eq(app.getId()), eq(StatusType.DECISION), eq(app.getHandler()));
