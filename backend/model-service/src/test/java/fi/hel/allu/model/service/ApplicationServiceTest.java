...
public class ApplicationServiceTest {
  ...
  public void testGetDeletableApplications_returnsCorrectData() {
    ...
    for (int i = 0; i < result.size(); i++) {
      DeletableApplication da = result.get(i);
      assertEquals(i + 1, da.getId().intValue());
      assertEquals("APP00" + (i + 1), da.getApplicationId());
      assertEquals(startTime, da.getStartTime());
      assertEquals(endTime, da.getEndTime());
      assertEquals(i == 0 ? ChangeType.CONTENTS_CHANGED : ChangeType.STATUS_CHANGED, da.getChangeType());
      assertEquals(changeTime, da.getChangeTime());
    }
