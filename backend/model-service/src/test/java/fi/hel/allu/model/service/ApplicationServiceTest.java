...
public class ApplicationServiceTest {
  ...
  @Test
  public void testGetDeletableApplications_returnsCorrectData() {
    ...
    List<DeletableApplication> mockApplications = List.of(
      new DeletableApplication(1, "APP001", startTime, endTime, ChangeType.CONTENTS_CHANGED, changeTime),
      new DeletableApplication(2, "APP002", startTime, endTime, ChangeType.STATUS_CHANGED, changeTime)
    );

    when(applicationDao.findDeletableApplications()).thenReturn(mockApplications);

    List<DeletableApplication> result = applicationService.getDeletableApplications();

    assertEquals(2, result.size());
    for (int i = 0; i < result.size(); i++) {
      DeletableApplication application = result.get(i);
      assertEquals(i + 1, application.getId());
      assertEquals("APP00" + (i + 1), application.getApplicationId());
      assertEquals(startTime, application.getStartTime());
      assertEquals(endTime, application.getEndTime());
      assertEquals(i == 0 ? ChangeType.CONTENTS_CHANGED : ChangeType.STATUS_CHANGED, application.getChangeType());
      assertEquals(changeTime, application.getChangeTime());
    }

    verify(applicationDao, times(1)).findDeletableApplications();
  }

  @Test
  public void testGetDeletableApplications_emptyList() {
    when(applicationDao.findDeletableApplications()).thenReturn(List.of());
