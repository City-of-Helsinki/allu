...
public class ApplicationAttachmentController {
		...
		@ApiResponses(value = {
						...
						@ApiResponse(responseCode = "400", description = "Invalid request data", content = @Content(schema =
		...
		public ResponseEntity<Void> create(
						...
						@Parameter(name = "Attachment info in JSON", required = true) @Valid @RequestPart(value = "metadata") AttachmentInfoExt metadata,
						@Parameter(name = "Attachment data", required = true) @RequestPart(value = "file") MultipartFile file) throws IOException {
		...
		@ApiResponses(value = {
						@ApiResponse(responseCode = "200", description = "Attachment data fetched successfully", content =
            @Content(schema = @Schema(implementation = ErrorInfo.class))),
		})
		@GetMapping(value = "/applications/{id}/attachments/{attachmentId}/data",
