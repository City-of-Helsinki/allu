...
public abstract class BaseApplicationController<T extends BaseApplicationExt, M extends ApplicationExtMapper<T>> {
    ...
    @Operation(summary = "Create new application",
            description = "Create new application. Returns ID of the created application. "
                    + "If application is still pending in client side, " +
                    "pendingOnClient should be set to true to prevent handling of application in Allu" +
                    " and to allow later modification of application data by client. " +
                    "If application is ready to be handled in Allu, pendingOnClient should be set to false.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<Integer> update(@Parameter(description = "Id of the application to update.")
        ...
        return new ResponseEntity<>(applicationService.updateApplication(applicationId, application, getMapper()),
                                    HttpStatus.OK);
    }
    ...
    public ResponseEntity<Void> addResponse(
            ...
            @Parameter(
                    description = "Content of the response") @RequestBody
            @Valid InformationRequestResponseExt<T> response) throws JsonProcessingException {
        applicationService.addInformationRequestResponse(
                applicationService.getApplicationIdForExternalId(applicationId),
                requestId, response, getMapper());
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @Operation(summary = "Report application changes.",
            description = "Only fields listed in change information are processed in Allu. "
                    + "Also data sent through some separate API (e.g. application attachments) should be included in " +
                    "field list of change. " +
    ...
    @PreAuthorize("hasAnyRole('ROLE_INTERNAL','ROLE_TRUSTED_PARTNER')")
    public ResponseEntity<Void> reportChange(
            @Parameter(description = "Id of the application") @PathVariable("applicationid") Integer applicationId,
            @Parameter(
                    description = "Contents of the change") @RequestBody
            @Valid InformationRequestResponseExt<T> change) throws JsonProcessingException {
        applicationService.reportApplicationChange(applicationService.getApplicationIdForExternalId(applicationId),
                                                   change,
                                                   getMapper());
        return new ResponseEntity<>(HttpStatus.OK);
    }
    ...
    @Operation(summary = "Gets decision metadata for application with given ID. " +
                    "If there's not yet decision for application, decision maker is null")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Decision metadata retrieved successfully",
                    content = @Content(schema =
