...
public class AttachmentController {
    ...
    @PreAuthorize("hasAnyRole('ROLE_SUPERVISE')")
    public ResponseEntity<Void> addDefaultAttachments(
            @Parameter(description = "Application ID to add default attachment for") @PathVariable Integer id,
            @Parameter(description = "Default attachment ID", required = true) @RequestBody
            List<Integer> defaultAttachmentIds) {
    ...
    public ResponseEntity<Void> delete(@Parameter(description = "Application ID") @PathVariable Integer id,
                                       @Parameter(description = "Attachment ID to delete", required = true)
    ...
    public ResponseEntity<List<AttachmentInfoJson>> getAttachments(
            @Parameter(description = "Application ID to fetch attachments for") @PathVariable Integer id) {
    ...
    @PreAuthorize("hasAnyRole('ROLE_SUPERVISE')")
    public ResponseEntity<AttachmentInfoJson> updateDecisionAttachment(
            @Parameter(description = "ID of updated attachment") @PathVariable Integer id,
    ...
    public ResponseEntity<List<AttachmentInfoJson>> readAttachmentInfos(@PathVariable ApplicationType applicationType) {
        List<AttachmentInfoJson> result = attachmentService.getDefaultAttachmentsByApplicationType(applicationType)
                .stream()
                .map(d -> new AttachmentInfoJson(d.getId(), d.getHandlerName(), d.getType(), d.getMimeType(),
                                                 d.getName(),
                                                 d.getDescription(), d.getSize(), d.getCreationTime(),
                                                 d.isDecisionAttachment()))
