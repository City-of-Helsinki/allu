...
public class GenericSearchService<T, Q extends QueryParameters> {
    ...
    public void initIndex(boolean checkVersion) {
        ...
        if (currentIndexName == null) {
            ...
            logger.debug("Current index name for {} is {} ({})", indexConductor.getIndexAliasName(), currentIndexName,
                         this);
    ...
    public void endSync() {
        if (indexConductor.tryDeactivateSync()) {
            try {
                updateAlias(indexConductor.getCurrentIndexName(), indexConductor.getNewIndexName(),
                            indexConductor.getIndexAliasName());
    ...
    private void updateAlias(String oldIndexName, String newIndexName, String alias) {
        ...
        client.admin().indices().prepareAliases().removeAlias(oldIndexName, alias).addAlias(newIndexName, alias)
                .execute().actionGet();
    }
    ...
    public void deleteIndex() {
        indexConductor.generateNewIndexName();
        initializeIndex(indexConductor.getNewIndexName());
        updateAlias(indexConductor.getCurrentIndexName(), indexConductor.getCurrentIndexName(),
                    indexConductor.getIndexAliasName());
        final String oldIndex = indexConductor.getCurrentIndexName();
        indexConductor.commitNewIndex();
        deleteIndex(oldIndex);
    ...
    private class BulkProcessorListener implements BulkProcessor.Listener {
        ...
        public void afterBulk(long executionId, BulkRequest request, BulkResponse response) {
            logger.debug(
                    "Bulk execution completed [" + executionId + "]. Took (ms): " + response.getIngestTookInMillis() + ". Failures: "
