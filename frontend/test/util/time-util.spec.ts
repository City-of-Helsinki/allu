...
describe('Time util', () => {
  it('should show is in winter time', () => {
    expect(TimeUtil.isInWinterTime(moment('2017-12-01').toDate()))
      .toBe(true, 'Winter start date was not in winter time');

    expect(TimeUtil.isInWinterTime(moment('2018-05-13').toDate()))
      .toBe(true, 'Winter end date was not in winter time');

    expect(TimeUtil.isInWinterTime(moment('2017-01-01').toDate()))
  ...
  it('should map given date to the end of winter time current year', () => {
    ...
    const winterTimeEnd = TimeUtil.toWinterTimeEnd(original.toDate());
    expect(winterTimeEnd).toEqual(WINTER_TIME_END.year(original.year()).toDate());
  });

  it('should map given date before year change to winter time end next year', () => {
    const original = moment('2016-12-12');
    const winterTimeEnd = TimeUtil.toWinterTimeEnd(original.toDate());
    expect(winterTimeEnd).toEqual(WINTER_TIME_END.year(original.year() + 1).toDate());
  });
});
