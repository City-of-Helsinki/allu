...
const DECISION_URL = '/api/applications/{id}/decision';
...
export class DecisionService {
  ...
  public generate(applicationId: number): Observable<Decision> {
    ...
    let url = DECISION_URL.replace('{id}', String(applicationId));
  ...
  public preview(applicationId: number): Observable<Decision> {
    ...
    let url = DECISION_PREVIEW_URL.replace('{id}', String(applicationId));
