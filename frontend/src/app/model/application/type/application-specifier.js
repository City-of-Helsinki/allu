...
})(ApplicationSpecifier = exports.ApplicationSpecifier || (exports.ApplicationSpecifier = {}));
var SpecifierEntry = /** @class */ (function () {
    function SpecifierEntry(specifier, kind) {
        this.specifier = specifier;
        this.kind = kind;
    }
    Object.defineProperty(SpecifierEntry.prototype, "key", {
        get: function () {
    ...
    SpecifierEntry.fromKey = function (key) {
        var kindAndSpecifier = key.split(':');
        return new SpecifierEntry(kindAndSpecifier[1], kindAndSpecifier[0]);
    };
    return SpecifierEntry;
}());
exports.SpecifierEntry = SpecifierEntry;
function toKindsWithSpecifiers(specifierEntries) {
    return specifierEntries.reduce(function (prev, cur) {
        var next = object_util_1.ObjectUtil.clone(prev);
...
exports.toKindsWithSpecifiers = toKindsWithSpecifiers;
/**
 * Converts kinds with specifiers object to array of kind:specifier keys
 * where string is in form of kind:specifier
 */
function fromKindsWithSpecifiers(kindsWithSpecifiers) {
    return Object.keys(kindsWithSpecifiers)
        .map(function (kind) { return fromKindAndSpecifiers(kind, kindsWithSpecifiers[kind]); })
        .reduce(function (prev, cur) { return prev.concat(cur); }, []);
}
exports.fromKindsWithSpecifiers = fromKindsWithSpecifiers;
function fromKindAndSpecifiers(kind, specifiers) {
    return specifiers.map(function (s) { return new SpecifierEntry(s, kind).key; });
}
