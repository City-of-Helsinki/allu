...
export class InformationAcceptanceResolve  {
            ...
            switchMap(request => {
              if (request && canHaveResponse(request.status)) {
                return this.informationRequestService.getResponseForRequest(request.informationRequestId).pipe(
                  switchMap(response => this.buildAcceptanceDataWithResponse(currentApp, pending, response)),
  ...
  private buildAcceptanceDataWithResponse(
  ...
  ): Observable<InformationAcceptanceData> {
    if (!response || !response.responseData) {
      return pendingFields.length ? of({ oldInfo: currentApp, newInfo: currentApp, updatedFields: pendingFields }) : EMPTY;
    }

    const responseClientData = response.responseData.clientApplicationData;
    const responseFields = response.updatedFields || [];
    ...
    // Re-evaluate pending fields after merge.
    // NOTE: getPendingDataFields only includes fields with truthy values, so it misses
    // null fields (i.e. explicit customer removals). We therefore must always union in
    // responseFields to ensure removed-customer fields are included even when null.
  ...
  /**
   * Returns field keys for which the clientData has a truthy (non-null, non-undefined) value.
   * NOTE: Explicit removals (null values) are intentionally excluded here. Callers that need to
   * include removed fields must union in the response's updatedFields afterward — see
   * buildAcceptanceDataWithResponse for the canonical example.
   */
