...
var toKey = function (path) {
    return option_1.Some(path).map(function (p) {
        var pathString = string_util_1.StringUtil.toPath(p, '.');
 ...
 */
exports.findTranslation = function (path) {
    return toKey(path)
        .map(function (pathParts) { return pathParts.reduce(function (acc, cur) { return option_1.Some(acc[cur]).orElse(pathParts.join('.')); }, exports.translations); })
        .orElse('');
};
/**
 ...
 */
exports.translateArray = function (pathPrefix, pathValues) {
    return pathValues.map(function (val) { return exports.findTranslation([pathPrefix, val]); });
};
