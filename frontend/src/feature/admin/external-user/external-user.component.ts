...
@Component({
  ...
  template: require('./external-user.component.html'),
  styles: [
    require('./external-user.component.scss')
  ]
})
export class ExternalUserComponent implements OnInit {
  ...
  save(): void {
    let user = ExternalUserForm.to(this.userForm.value);
    this.submitted = true;
  ...
  addCustomer(customerId: number): void {
    let current = this.connectedCustomersCtrl.value;
